...
io.on("connection", (socket) => {
    ...
    socket.on("joined", () => {
        socket.emit("receive-message", { message: "Hi, I am Jennifer. Your Personal Assistant for IQ Canada related queries...How may I help you today?" });
    });
    ...
    socket.on("send-message", ({ message }) => {
        ...
        for (let [key, value] of Object.entries(responses)) {
            if (message.toLowerCase().includes(key.toLowerCase())) {
                socket.emit("receive-message", { message: value });
                return;
            }
        }

        socket.emit("receive-message", { message: "Sorry, I didn't get that. Please try again." });
    });
});
